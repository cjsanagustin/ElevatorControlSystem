...
{
    public class UpwardMovementState : ElevatorMovementState
    {
        public override void MoveOneFloor(Elevator elevator)
        {
            if (elevator.IsMoving)
            {
                var nextRequest = elevator.RequestQueue.Where(x => x.DestinationFloor > elevator.CurrentFloor)
                    .OrderBy(x => x.DestinationFloor).ToList().FirstOrDefault();
                if (nextRequest != null)
                {
        ...
        /// <summary>
        /// Change elevator direction if there are one or more downward request and no upward request
        /// </summary>
        ...
        public override void ChangeDirection(Elevator elevator)
        {
            ...
            _context?.TransitionTo(new DownwardMovementState());
